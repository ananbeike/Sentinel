...
public class SentinelGatewayFilter implements GatewayFilter,GlobalFilter,Ordered{
    ...
    public Mono<Void> filter(ServerWebExchange exchange,GatewayFilterChain chain){
        ...
        if (route != null){
            ...
            Object[] params = paramParser
                            .parseParameterFor(routeId, exchange, r -> r.getResourceMode() == SentinelGatewayConstants.RESOURCE_MODE_ROUTE_ID);
            ...
            EntryConfig entryConfig = new EntryConfig(
                            ...
                            1,
                            params,
                            new ContextConfig(contextName(routeId), origin));
            SentinelReactorTransformer<Void> transformer = new SentinelReactorTransformer<>(entryConfig);

            asyncResult = asyncResult.transform(transformer);
        }

        Set<String> matchingApis = pickMatchingApiDefinitions(exchange);
        for (String apiName : matchingApis){

            Predicate<GatewayFlowRule> gatewayFlowRulePredicate = r -> r.getResourceMode() == SentinelGatewayConstants.RESOURCE_MODE_CUSTOM_API_NAME;

            Object[] params = paramParser.parseParameterFor(apiName, exchange, gatewayFlowRulePredicate);

            EntryConfig entryConfig = new EntryConfig(apiName, ResourceTypeConstants.COMMON_API_GATEWAY, EntryType.IN, 1, params);
            SentinelReactorTransformer<Void> transformer = new SentinelReactorTransformer<>(entryConfig);
    ...
    Set<String> pickMatchingApiDefinitions(ServerWebExchange exchange){
        return GatewayApiMatcherManager.getApiMatcherMap().values().stream().filter(m -> m.test(exchange)).map(WebExchangeApiMatcher::getApiName)
